...
public class dbConnectivity {
...

    {
        ...
        // Connection string
        String url = "jdbc:sqlserver://localhost:3306;databaseName=library";
    ...
    void ChangeBookInfo(int book_id, String UpdatedInfo, int type) {
        ...
        try {
            if (type == 1) {
                int i = stmt
                        .executeUpdate("UPDATE Books  SET title='" + UpdatedInfo + "'Where book_id='" + book_id + "'");
            } else if (type == 2) {
                int i = stmt
                        .executeUpdate("UPDATE Books  SET author='" + UpdatedInfo + "'Where book_id='" + book_id + "'");
            } else if (type == 3) {
                int i = stmt.executeUpdate(
                        "UPDATE Books  SET subject='" + UpdatedInfo + "'Where book_id='" + book_id + "'");
            }
        } catch (Exception e) {
            System.out.println(e);
        }
    ...
    Borrower GetaBorrowerObjectByUserId(int id) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Borrower TempBorrowerObject = new Borrower(borrower_id, borrower_name, borrower_gender,
                        borrower_address, borrower_number, fine_defaulter, fine);
    ...
    ArrayList<Users> LoadAllBorrowers() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Borrower TempBorrowerObj = new Borrower(borrower_id, borrower_name, borrower_gender, borrower_number,
                        borrower_address, fine_defaulter, fine);
    ...
    boolean SetFineStatus(int borrower_id, boolean fine_defaulter) {
        ...
        try {
            stmt.executeUpdate("Update Borrower Set fine_defaulter='" + fine_defaulter + "' Where borrower_id='"
                    + borrower_id + "'");
    ...
    boolean SetTelephone(int borrower_id, String telnum) {
        ...
        try {
            stmt.executeUpdate(
                    "Update Borrower Set borrower_num='" + telnum + "' Where borrower_id='" + borrower_id + "'");
    ...
    boolean SetAddress(int borrower_id, String Address) {
        ...
        try {
            stmt.executeUpdate(
                    "Update Borrower Set borrower_address='" + Address + "' Where borrower_id='" + borrower_id + "'");
    ...
    boolean SetName(int borrower_id, String name) {
        ...
        try {
            stmt.executeUpdate(
                    "Update Borrower Set borrower_name='" + name + "' Where borrower_id='" + borrower_id + "'");
    ...
    boolean SetGender(int borrower_id, char g) {
        ...
        try {
            stmt.executeUpdate(
                    "Update Borrower Set borrower_gender='" + g + "' Where borrower_id='" + borrower_id + "'");
    ...
    boolean SetFineAmount(int borrower_id, double fine_amount) {
        ...
        try {
            stmt.executeUpdate(
                    "Update Borrower Set fine='" + fine_amount + "' Where borrower_id='" + borrower_id + "'");
    ...
    boolean GetFineStatus(int borrower_id) {
        ...
        try {
            ResultSet rs = stmt
                    .executeQuery("select fine_defaulter from Borrower where borrower_id='" + borrower_id + "'");
    ...
    boolean AddBorrower(int user_id, String user_name, char user_gender, String add, String telnum) {
        ...
        try {
            ...
            stmt.executeUpdate(
                    "Insert into Borrower (borrower_id, borrower_name ,borrower_gender, borrower_address , borrower_number , fine_defaulter ,fine) values('"
                            + user_id + "','" + user_name + "','" + user_gender + "','" + add + "','" + telnum + "','"
                            + fine + "','" + fineamount + "')");
    ...
    ArrayList<Loan> LoadLoanList() {
        ...
        try {
            ResultSet rs = stmt.executeQuery(
                    "Select * from (Borrower join Loan as L on Borrower.borrower_id=L.borrower_id)  join Books on Books.book_id=L.book_id");
            while (rs.next()) {
                int borrower_id;
                String borrower_name, borrower_address, borrower_number;
                char borrower_gender;
                ...
                Borrower Loanee = new Borrower(borrower_id, borrower_name, borrower_gender, borrower_number,
                        borrower_address, fine_defaulter, fine);
                Books LoanedBook = new Books(book_id, author, title, subject, quantity);
                Loan LoanTempObj = new Loan(loan_id, Loanee, LoanedBook, fine_status, returned_status, issue_date,
    ...
    ArrayList<Loan> LoadLoanListofSpecificUser(int user_id) {
        ...
        try {
            ResultSet ds = stmt.executeQuery(
                    "Select * from (Borrower join Loan as L on Borrower.borrower_id=L.borrower_id)  join Books on Books.book_id=L.book_id where Borrower.borrower_id='"
                            + user_id + "'");
            while (ds.next()) {
                int borrower_id;
                String borrower_name, borrower_address, borrower_number;
    ...
    boolean AddNewLoan(Loan LoanObj) {
        ...
        try {
            ...
            stmt.executeUpdate(
                    "Insert into Loan (loanid , issue_date ,due_date ,  return_date , fine_status , returned_status ,borrower_id ,book_id ) values('"
                            + loanid + "','" + issue_date1 + "','" + due_date1 + "','" + return_date1 + "','"
                            + fine_status + "','" + returned_status + "','" + borrower_id + "','" + book_id + "')");
    ...
    void SetLoan(int loanID, Loan LoanObj) {
        ...
        try {
            ...
            stmt.executeUpdate("Update Loan SET issue_date='" + issue_date1 + "',due_date='" + due_date1
                    + "' ,return_date='" + return_date1 + "' , fine_status='" + fine_status + "', returned_status= '"
                    + returned_status + "',borrower_id='" + borrower_id + "' ,book_id='" + book_id
                    + "'  Where loanid=' " + loanid + "'");
        } catch (Exception e) {
            System.out.println(e);
        }
    ...
    void AddNewBook(int book_id, String title, String author, String subject, int quantity) {
        ...
        try {
            ...
            stmt.executeUpdate("Insert into Books (book_id, author ,title, subject , quantity) values('" + book_id
                    + "','" + author + "','" + title + "','" + subject + "','" + quantity + "')");
    ...
    boolean CheckIsBookLoaned(int book_id) {
        ...
        try {
            ...
            ResultSet rs = stmt.executeQuery(
                    "select * from Loan where book_id='" + book_id + " 'and returned_status ='" + 0 + "'");
            if (rs.next()) {
